    ...
    fn from_bytes(input: &[u8]) -> std::io::Result<RedisMessage> {
        ...
        let msg = match bytes.next() {
            Some(b'*') => {
                ...
                let len = len.parse::<usize>().map_err(|_err| {
                    std::io::Error::new(
                        std::io::ErrorKind::InvalidData,
                        format!("{len} is not a number"),
                    )
                })?;

                let mut items = Vec::with_capacity(len);
                for _ in 0..len {
                    let item = RedisMessage::from_bytes(&bytes.as_ref())?;
                    items.push(item);
                }

                RedisMessage::Array(items)
            }
            Some(b'+') => {
            ...
            Some(b'$') => {
                ...
                let bytes_to_read = header.parse::<usize>().map_err(|_err| {
                    std::io::Error::new(
                        std::io::ErrorKind::InvalidData,
                        format!("{header} is not a number"),
                    )
                })?;
