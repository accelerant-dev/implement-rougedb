...
fn bulk_strings() {
    let input = b"$7\r\nrougedb\r\n";
    let s = to_bulk_string(input).unwrap();
    assert_eq!(&s, b"rougedb");
}

/// Assumes that the prefix byte (`b'$'`) has
/// already been stripped
fn to_bulk_string(input: &[u8]) -> std::io::Result<Vec<u8>> {
    // Strip $ prefix when present
    let start = if input.first() == Some(&b'$') {
        1
    } else {
        0
    };

    let end = input[start..].iter()
        .take_while(|x| x.is_ascii_digit()).count();
    let len = std::str::from_utf8(&input[start..(start+end)])
    .map_err(|_|std::io::Error::new(std::io::ErrorKind::InvalidData, "unable to parse input"))?
    .parse::<usize>()
    .map_err(|_|std::io::Error::new(std::io::ErrorKind::InvalidData, "unable to parse input"))?;

    let data = &input[(start + end + 2)..(start + end + 2 + len)];
    Ok(data.to_vec())
}
...
impl RedisMessage {
    fn from_bytes(input: &[u8]) -> std::io::Result<RedisMessage> {
        let mut bytes = input.iter().peekable();
        let mut payload = Vec::with_capacity(128);

        let msg = match bytes.next() {
            Some(b'+') => {
...
fn main() -> std::io::Result<()> {
    ...
    let msg = RedisMessage::from_bytes(&buffer)?;
